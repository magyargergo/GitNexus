...
| `GITNEXUS_SWIFT_MAX_MODULE_FILES`               | `1000`                             | Files per Swift module (SwiftPM target, or the `__default__` bucket for files outside any target) above which the pairwise sibling-visibility passes skip that module with a warning. Calls in a skipped module resolve through the lower-confidence global name fallback. Non-negative integer; invalid values use the default.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Indexing a Swift app with one very large module: raise it on a machine with heap to spare (about 3.8 GB extra at 1,000 files); lower it if analyze runs out of memory in scope resolution. |
